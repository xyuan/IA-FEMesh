...
Module:    $RCSfile: itkInverseConsistentLandmarks.txx,v $
Language:  C++
        ...
        ::ReadIntellXPointTypes( const std::string lmrkfilename,
            ...
            for(int p=0; p<NumberOfPoints; p++)
                ...
                for(unsigned int iter = 0; iter < CurrentLandmarkName.length(); iter++ )
                    {
                    //std::cout << "CurrentLandmarkName[" << iter << "] = " << CurrentLandmarkName[iter] << std::endl;
                    if(CurrentLandmarkName[iter] == '\n'||CurrentLandmarkName[iter] == '\r'||CurrentLandmarkName[iter] == '\t'||CurrentLandmarkName[iter] == '\"')
                        {
                        continue;
                        }
                    else if(CurrentLandmarkName[iter] == ' ')
    ...
    template <typename PointStorageType,typename PointSetType>
        ...
        ::ReadIPLPointTypes( const std::string lmrkfilename )
            ...
            while(fgets(buffer, FILE_BUFFER_SIZE, tempfile ) != NULL)
                ...
                TempPnt.SetT(0.0F);
                TempPnt.SetWeighting(1.0F);
                if((ImageDims[0]<=TempPnt[0]*ImageRes[0])||(ImageDims[1]<=TempPnt[1]*ImageRes[1])||(ImageDims[2]<=TempPnt[2]*ImageRes[2])||(ImageDims[3]<=TempPnt.GetT()*ImageRes[3]))
                    {
    ...
    template <typename PointStorageType,typename PointSetType>
        ...
        ::process_bnd_point(const std::string & CurrentLandmarkName, const char * buffer,
            ...
            if(local_ImageDims[0]<=ModifiedPoint[0])
                {
                //std::cout << "Error: Point "<< CurrentLandmarkName << " has invalid x coordinate.  Max value is: " << local_ImageDims[0]-1<<std::endl;
                //exit( -1 );
                return false;
                }
            if(local_ImageDims[1]<=ModifiedPoint[1])
                {
                //std::cout << "Error: Point "<< CurrentLandmarkName << " has invalid y coordinate.  Max value is: " << local_ImageDims[1]-1<<std::endl;
                //exit( -1 );
                return false;
                }
            if(local_ImageDims[2]<=ModifiedPoint[2])
                {
                //std::cout << "Error: Point "<< CurrentLandmarkName << " has invalid z coordinate.  Max value is: " << local_ImageDims[2]-1<<std::endl;
                //exit( -1 );
                return false;
                }
            if(local_ImageDims[3]<=ModifiedPoint.GetT())
                {
                //std::cout << "Error: Point "<< CurrentLandmarkName << " has invalid t coordinate.  Max value is: " << local_ImageDims[3]-1<<std::endl;
                //exit( -1 );
                return false;
                }
            for(int i=0; i < 3; i++)
        ...
        ::ReadIPLTalairachPointTypes( const std::string lmrkfilename )
            ...
            //First read the SLA point
                ...
                PointType TempPnt=SLAPoint;
                    {
                    std::cout << "  Coordinates:: SLA " << SLAPoint << "   IRP " << IRPPoint << "   PC " << PC_Point << "   AC " << AC_Point << std::endl;
                    std::string CurrentLandmarkName ="AC-Point";    TempPnt[0]=AC_Point[0]; TempPnt[1]=AC_Point[1]; TempPnt[2]=AC_Point[2]; (*this)[CurrentLandmarkName]=TempPnt;
                    CurrentLandmarkName="PC-Point";    TempPnt[0]=PC_Point[0]; TempPnt[1]=PC_Point[1]; TempPnt[2]=PC_Point[2]; (*this)[CurrentLandmarkName]=TempPnt;

                    /* Inferior Points */
                    CurrentLandmarkName="IRP";    TempPnt[0]=IRPPoint[0]; TempPnt[1]=IRPPoint[1]; TempPnt[2]=IRPPoint[2]; (*this)[CurrentLandmarkName]=TempPnt;
                    CurrentLandmarkName="IRA";    TempPnt[0]=IRPPoint[0]; TempPnt[1]=IRPPoint[1]; TempPnt[2]=SLAPoint[2]; (*this)[CurrentLandmarkName]=TempPnt;
                    CurrentLandmarkName="ILP";    TempPnt[0]=SLAPoint[0]; TempPnt[1]=IRPPoint[1]; TempPnt[2]=IRPPoint[2]; (*this)[CurrentLandmarkName]=TempPnt;
                    CurrentLandmarkName="ILA";    TempPnt[0]=SLAPoint[0]; TempPnt[1]=IRPPoint[1]; TempPnt[2]=SLAPoint[2]; (*this)[CurrentLandmarkName]=TempPnt;

                    /* Superior Points */
                    CurrentLandmarkName="SLA";    TempPnt[0]=SLAPoint[0]; TempPnt[1]=SLAPoint[1]; TempPnt[2]=SLAPoint[2]; (*this)[CurrentLandmarkName]=TempPnt;
                    CurrentLandmarkName="SLP";    TempPnt[0]=SLAPoint[0]; TempPnt[1]=SLAPoint[1]; TempPnt[2]=IRPPoint[2]; (*this)[CurrentLandmarkName]=TempPnt;
                    CurrentLandmarkName="SRA";    TempPnt[0]=IRPPoint[0]; TempPnt[1]=SLAPoint[1]; TempPnt[2]=SLAPoint[2]; (*this)[CurrentLandmarkName]=TempPnt;
                    CurrentLandmarkName="SRP";    TempPnt[0]=IRPPoint[0]; TempPnt[1]=SLAPoint[1]; TempPnt[2]=IRPPoint[2]; (*this)[CurrentLandmarkName]=TempPnt;
    ...
    template <typename PointStorageType,typename PointSetType>
        ...
        ::ReadIPLCerebellarPointTypes( const std::string lmrkfilename )
            ...
            //First read the SLA point
                ...
                status=fgets(buffer, FILE_BUFFER_SIZE, tempfile );
                if(status==NULL) { std::cout << "ERROR: End of file reached before TALAIRACH_PARAMETER_HEADER_END found" << std::endl; }
                if(process_bnd_point("SLAPoint", buffer, ImageDims, ImageRes, SLAPoint)==false) {return false;}
                    {//NOTE: for Cerebellum AC_Point is meaningless
                    PointType AC_Point;  //Antieror Commisure
                    status=fgets(buffer, FILE_BUFFER_SIZE, tempfile );
                    if(status==NULL) { std::cout << "ERROR: End of file reached before TALAIRACH_PARAMETER_HEADER_END found" << std::endl; }
                    //if(process_bnd_point("AC_Point", buffer, ImageDims, ImageRes, AC_Point)==false) {return false;}
                    }

                PointType FV_Point;  //Posterior Commisure
                status=fgets(buffer, FILE_BUFFER_SIZE, tempfile );
                if(status==NULL) { std::cout << "ERROR: End of file reached before TALAIRACH_PARAMETER_HEADER_END found" << std::endl; }
                if(process_bnd_point("FV_Point", buffer, ImageDims, ImageRes, FV_Point)==false) {return false;}

                PointType IRPPoint; //Inferior Right Posterior
                status=fgets(buffer, FILE_BUFFER_SIZE, tempfile );
                if(status==NULL) { std::cout << "ERROR: End of file reached before TALAIRACH_PARAMETER_HEADER_END found" << std::endl; }
                if(process_bnd_point("IRPPoint", buffer, ImageDims, ImageRes, IRPPoint)==false) {return false;}
                //Process to extend 4 points to all possible combinations
                PointType TempPnt=SLAPoint;
                    {
                    std::string CurrentLandmarkName;
