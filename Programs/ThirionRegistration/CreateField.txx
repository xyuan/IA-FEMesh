...
namespace itk
{
    template <typename TImage, typename T2Image> CreateField <TImage, T2Image>::CreateField()
        {
    ...
    template <typename TImage, typename T2Image> void CreateField <TImage, T2Image>::Execute()
        {
        try
            {
            std::cout << "  Reading Input Images and Parameters" << std::endl;
            m_ImageOne= itkUtil::ReadImageCoronal<TImage>( m_Image1FileName );
            m_ImageTwo= itkUtil::ReadImageCoronal<TImage>( m_Image2FileName );
        ...
        try
            ...
            //HACK:  Need to fixe DirectionType != operator to be const correct if(m_FixedImage->GetDirection() != m_MovingImage->GetDirection()
            typename TImage::DirectionType temp=m_FixedImage->GetDirection();
            if(temp != m_MovingImage->GetDirection()
                ||
                (itk::SpatialOrientationAdapter().FromDirectionCosines(  m_FixedImage->GetDirection()) != itk::SpatialOrientation::ITK_COORDINATE_ORIENTATION_RIP)
            )
                {
            ...
            try
                ...
                //Hardcode orientation to RIP ased on m_FixedImage image
    ...
    template <typename TImage, typename T2Image> void CreateField <TImage, T2Image>::ReleaseDataFlagOn()
        {
    ...
    template <typename TImage, typename T2Image> void CreateField<TImage,T2Image>::NormalizeImage(InputImageType * input,
    ...
    template < typename OutputImageType , typename InputImageType > void FFCreateNewImageFromTemplate(typename OutputImageType::Pointer & PointerToOutputImage,
    ...
    template <typename TImage, typename T2Image> void CreateField<TImage,T2Image>::StartNewLevel()
        {
