...
vtkCxxRevisionMacro(vtkKWMimxEvaluateMeshQualityGroup, "$Revision: 1.48.2.2 $");
...
int vtkKWMimxEvaluateMeshQualityGroup::EvaluateMeshQualityApplyCallback(int qualityType)
{
  ...
  if (this->QualityType == MESH_QUALITY_MAX_ANGLE)
    meshActor->SetColorRangeType(vtkMimxMeshActor::BlueToRed, qualityName, NULL, meshActor->ComputeMeshScalarRange(qualityName), true);
  else
    meshActor->SetColorRangeType(vtkMimxMeshActor::RedToBlue, qualityName, NULL, meshActor->ComputeMeshScalarRange(qualityName), true);
  
  meshActor->SetLegendTitle( qualityName );
  //meshActor->SetMeshScalarName( qualityName );
  meshActor->SetMeshScalarVisibility(true);
  meshActor->SetAllElementSetScalarName( qualityName );
  ...
  if(this->ViewOptionsGroup)
  {
	...
	//this->ViewOptionsGroup->ColorModeCallback(1);
	//this->ViewOptionsGroup->ColorModeCallback(2);
  }
 /* if(meshActor->GetColorRangeType() == vtkMimxMeshActor::BlueToRed)
  {
	  meshActor->BlueToRedLookUpTable();
  }
  else
	meshActor->RedToBlueLookUpTable();*/
  this->GetMimxMainWindow()->GetRenderWidget()->Render();
...
void vtkKWMimxEvaluateMeshQualityGroup::UpdateObjectLists()
{
  ...
  if (defaultItem != -1)
  {
    ...
    strcpy(this->PreviousSelection, this->FEMeshList->GetItem(defaultItem)->GetFileName());
    this->SelectionChangedCallback(this->FEMeshList->GetItem(defaultItem)->GetFileName());
