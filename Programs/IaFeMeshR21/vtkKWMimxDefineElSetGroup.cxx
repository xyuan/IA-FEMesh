...
vtkCxxRevisionMacro(vtkKWMimxDefineElSetGroup, "$Revision: 1.30 $");

//----------------------------------------------------------------------------
class vtkWindowCloseEventCallback : public vtkCommand
{
public:
	static vtkWindowCloseEventCallback *New() 
	{ return new vtkWindowCloseEventCallback; }
	virtual void Execute(vtkObject *caller, unsigned long, void*)
	{
		this->DefineElementSetGroupInstance->CloseWindowCallback();
	}

	vtkKWMimxDefineElSetGroup *DefineElementSetGroupInstance;
};
//----------------------------------------------------------------------------
vtkKWMimxDefineElSetGroup::vtkKWMimxDefineElSetGroup()
{
  ...
  this->MeshModeButtonSet = NULL;
  this->SelectCellsWidget = NULL;
  this->SelectionState = -1;
  this->MeshActor = NULL;
  //this->AddSubtractButton = NULL;
  this->WindowCloseEventCallback = vtkWindowCloseEventCallback::New();
  this->WindowCloseEventCallback->DefineElementSetGroupInstance = this;
  this->AddObserver(vtkKWTopLevel::WithdrawEvent, this->WindowCloseEventCallback, 1.0);
  this->MeshTypeMenuButton = NULL;
  this->AddSubtractButton = NULL;
...
vtkKWMimxDefineElSetGroup::~vtkKWMimxDefineElSetGroup()
{
  ...
  if (this->ApplyButton)
	...
	if (this->MeshModeButtonSet)
	  this->MeshModeButtonSet->Delete();
	if (this->SelectCellsWidget)
	  this->SelectCellsWidget->Delete();
	//if(this->AddSubtractButton)
	//	this->AddSubtractButton->Delete();
	this->RemoveObservers(vtkKWTopLevel::WithdrawEvent, this->WindowCloseEventCallback);
	this->WindowCloseEventCallback->Delete();
	if(this->MeshTypeMenuButton)
		this->MeshTypeMenuButton->Delete();
	if (this->AddSubtractButton)
		this->AddSubtractButton->Delete();
...
void vtkKWMimxDefineElSetGroup::CreateWidget()
{
 ...
 /* if (!this->MeshTypeMenuButton)	
		this->MeshTypeMenuButton = vtkKWMenuButtonWithLabel::New();
	this->MeshTypeMenuButton->SetParent(this->ModeFrame);
	this->MeshTypeMenuButton->Create();
	this->MeshTypeMenuButton->SetBorderWidth(0);
	this->MeshTypeMenuButton->SetReliefToGroove();
	this->MeshTypeMenuButton->SetLabelText("Mesh :");
	this->MeshTypeMenuButton->SetPadX(2);
	this->MeshTypeMenuButton->SetPadY(2);
	this->MeshTypeMenuButton->GetWidget()->SetWidth(20);
	this->GetApplication()->Script("pack %s -side top -anchor n -padx 2 -pady 6", 
		this->MeshTypeMenuButton->GetWidgetName());

	this->MeshTypeMenuButton->GetWidget()->GetMenu()->AddRadioButton(
		"Full Mesh",this, "FullPartialSelectedMeshCallback 0");
	this->MeshTypeMenuButton->GetWidget()->GetMenu()->AddRadioButton(
		"Partial Mesh",this, "FullPartialSelectedMeshCallback 1");
	this->MeshTypeMenuButton->GetWidget()->SetValue("Full Mesh");*/
...
//		this->SelectButtonSet->GetWidget(3)->SetEnabled(0);
		...
		//		this->SelectButtonSet->GetWidget(4)->SetEnabled(0);
	}
	else
	{
		this->GetApplication()->Script("pack forget %s", this->ExtrudeFrame->GetWidgetName());
	}
  //this->SelectButtonSet->GetWidget(0)->SetSelectedState(1);
...
void vtkKWMimxDefineElSetGroup::SelectElementsSurfaceCallback(int mode)
{
  ...
  {
  //  this->DisableElementSelection(1);
  }
}
//---------------------------------------------------------------------------------
void vtkKWMimxDefineElSetGroup::SelectVisibleElementsSurfaceCallback(int mode)
{
  ...
  {
  //  this->DisableElementSelection(2);
  }
}
//---------------------------------------------------------------------------------
...
//    this->DisableElementSelection(3);
  }
}
//---------------------------------------------------------------------------------
void vtkKWMimxDefineElSetGroup::SelectMultipleElementsCallback(int mode)
{
	...
	{
//		this->DisableElementSelection(4);
	}
}
//---------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------
void vtkKWMimxDefineElSetGroup::FullCallback()
{
	if (this->SelectCellsWidget)
	{
		vtkUnstructuredGrid *ugrid = this->MeshActor->GetDataSet();
...
//---------------------------------------------------------------------------------
void vtkKWMimxDefineElSetGroup::AcceptSelectionCallback()
{
...
//---------------------------------------------------------------------------------------
void vtkKWMimxDefineElSetGroup::CloseWindowCallback()
{
	if(!this->SelectButtonSet)	return;
	...
	this->DisableElementSelection(5);
	this->GetSelectedObject()->ShowMesh();
}
//---------------------------------------------------------------------------------------
void vtkKWMimxDefineElSetGroup::FullPartialSelectedMeshCallback(int mode)
{
	vtkMimxErrorCallback *callback = this->GetMimxMainWindow()->GetErrorCallback();

	if(!this->SelectCellsWidget)
	{
		callback->ErrorMessage("Element selection not active");
		this->MeshTypeMenuButton->GetWidget()->SetValue("Full Mesh");
		return;
	}
	if(mode == vtkMimxCreateElementSetWidgetFEMesh::FullMesh)
	{
		this->SelectCellsWidget->SetEditMeshState(mode);
		this->SelectButtonSet->GetWidget(3)->SetEnabled(1);
	}
	else
	{
		int numCells = this->SelectCellsWidget->GetSelectedGrid()->GetNumberOfCells();
		if(!numCells)
		{
			callback->ErrorMessage("Number of Elements in the partial mesh is 0");
			this->MeshTypeMenuButton->GetWidget()->SetValue("Full Mesh");
			return;
		}
		else
