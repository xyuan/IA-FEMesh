...
vtkCxxRevisionMacro(vtkKWMimxCreateFEMeshFromBBGroup, "$Revision: 1.74.2.2 $");
...
void vtkKWMimxCreateFEMeshFromBBGroup::CreateWidget()
{
  ...
  //this->ExtrudeFrame->SetParent(this->MainFrame);
...
/*
  if(!this->RecalculationIterationsEntry)
   this->RecalculationIterationsEntry = vtkKWEntryWithLabel::New();
  this->RecalculationIterationsEntry->SetParent(this->MeshImprovementFrame->GetFrame());
  this->RecalculationIterationsEntry->Create();
  this->RecalculationIterationsEntry->SetWidth(5);
  this->RecalculationIterationsEntry->SetLabelText("Recalculation Iterations : ");
  this->RecalculationIterationsEntry->SetLabelWidth(30);
  this->RecalculationIterationsEntry->GetWidget()->SetValueAsInt(10);
  this->RecalculationIterationsEntry->GetWidget()->SetRestrictValueToInteger();
  this->GetApplication()->Script(
   "pack %s -side top -anchor nw -expand n -padx 2 -pady 6 -fill x", 
   this->RecalculationIterationsEntry->GetWidgetName());
*/
...
/*    
  if (!this->SelectFrame)	
     this->SelectFrame = vtkKWFrame::New();
  this->SelectFrame->SetParent(this->MainFrame);
  ...
  if (!this->SelectSurfaceButton)	
     this->SelectSurfaceButton = vtkKWRadioButton::New();
  this->SelectSurfaceButton->SetParent( this->SelectFrame );
  this->SelectSurfaceButton->Create();
  this->SelectSurfaceButton->SetValueAsInt(1);
  this->SelectSurfaceButton->SetImageToIcon( surfaceIcon );
  this->SelectSurfaceButton->SetSelectImageToIcon( surfaceIcon );
  this->GetApplication()->Script(
    "pack %s -side left -anchor n -padx 2 -pady 2", 
    this->SelectSurfaceButton->GetWidgetName()); 
  ...
  if (!this->SelectFaceButton)	
     this->SelectFaceButton = vtkKWRadioButton::New();
  this->SelectFaceButton->SetParent( this->SelectFrame );
  this->SelectFaceButton->Create();
  this->SelectFaceButton->SetValueAsInt(1);
  this->SelectFaceButton->SetImageToIcon( faceIcon );
  this->SelectFaceButton->SetSelectImageToIcon( faceIcon );
  this->GetApplication()->Script(
    "pack %s -side left -anchor n -padx 2 -pady 2", 
    this->SelectFaceButton->GetWidgetName()); 
  ...
  if (!this->SelectElementButton)	
     this->SelectElementButton = vtkKWRadioButton::New();
  this->SelectElementButton->SetParent( this->SelectFrame );
  this->SelectElementButton->Create();
  this->SelectElementButton->SetValueAsInt(1);
  this->SelectElementButton->SetImageToIcon( elementIcon );
  this->SelectElementButton->SetSelectImageToIcon( elementIcon );
  this->GetApplication()->Script(
    "pack %s -side left -anchor n -padx 2 -pady 2", 
    this->SelectElementButton->GetWidgetName()); 
*/    
...
int vtkKWMimxCreateFEMeshFromBBGroup::CreateFEMeshFromBBApplyCallback()
{
  ...
  if ( this->SmoothButton->GetWidget()->GetSelectedState() )
  {
    ...
    /*
    recalcIterations = this->RecalculationIterationsEntry->GetWidget()->GetValueAsInt();
    if( recalcIterations < 1 )
    {
      callback->ErrorMessage("Number of interior node calculation iterations must be greater than 0.");
      return 0;
    }
    */  	
  }  	  
  ...
  int result = 0;
 /*  double ExtrusionLength = this->ExtrudeLengthEntry->GetWidget()->GetValueAsDouble();
  int NumberOfDivisions = this->NumberOfDivisionsEntry->GetWidget()->GetValueAsInt();*/
  //double ExtrusionLength = this->DefineElementSetDialog->GetExtrusionLength();
  //int NumberOfDivisions = this->DefineElementSetDialog->GetNumberOfDivisions();
  double ExtrusionLength;
  int NumberOfDivisions;
  int meshingMode = this->GetMeshingMode();
  /*
  std::cout << "Mesh Mode " << meshingMode << std::endl;      
  std::cout << "Smooth Mode " << smoothFlag << std::endl;      
  std::cout << "\tIterations " << smoothingIterations << std::endl;  
  std::cout << "\tRecalc Iterations " << recalcIterations << std::endl;      
  std::cout << "Node Set Name " << nodesetname << std::endl; 
  std::cout << "Element Set Name " << elementsetname << std::endl; 
  std::cout << "Node Set Number " << nodeNumber << std::endl; 
  std::cout << "Element Set Number " << elementNumber << std::endl; 
  */        
  switch ( meshingMode )
  {
...
void vtkKWMimxCreateFEMeshFromBBGroup::UpdateObjectLists()
{
	...
	if (defaultItem != -1)
  {
	...
	grid = vtkMimxMeshActor::SafeDownCast(this->FEMeshList->GetItem(defaultItem))->GetDataSet();
  }
  else
  {
...
*/
  //std::cout << "Update Object List- Calc starting Node/Elemenmt	Number" << std::endl;
...
void vtkKWMimxCreateFEMeshFromBBGroup::VolumetricMeshExtrudeCallback()
{
  ...
  //  this->ExtrudeFrame->GetWidgetName(), this->ComponentFrame->GetWidgetName());
  this->GetApplication()->Script(
    "pack %s -side top -anchor n -padx 2 -pady 2 -after %s", 
...
void vtkKWMimxCreateFEMeshFromBBGroup::SmoothFEMeshCallback(int state)
{
	if (state)
	{
  	//this->RecalculationIterationsEntry->GetWidget()->SetStateToNormal();
  	this->SmoothingIterationsEntry->GetWidget()->SetStateToNormal();
	}
	else
	{
  	//this->RecalculationIterationsEntry->GetWidget()->SetStateToDisabled();
	...
	int numSmooth = this->SmoothingIterationsEntry->GetWidget()->GetValueAsInt();
	if(numSmooth < 1)
	{
		callback->ErrorMessage("Smoothing iterations should atleast be 1");
		return;
	}

	int numRecalc = this->RecalculationIterationsEntry->GetWidget()->GetValueAsInt();
	if(numRecalc < 1)
	{
		callback->ErrorMessage("Recalculation iteration should at least be 1");
		return;
	}

	int numItems = this->FEMeshList->GetNumberOfItems();
	vtkUnstructuredGrid *femesh = vtkMimxMeshActor::SafeDownCast(
		this->FEMeshList->GetItem(numItems-1))->GetDataSet();
	
	vtkMimxSmoothUnstructuredGridFilter *smooth = vtkMimxSmoothUnstructuredGridFilter::New();
	smooth->SetSource(polydata);
	smooth->SetInput(femesh);
	smooth->SetNumberOfIterations(numSmooth);
	smooth->AddObserver(vtkCommand::ErrorEvent, callback, 1.0);
	smooth->Update();
	if (!callback->GetState())
	{
		femesh->GetPoints()->DeepCopy(smooth->GetOutput()->GetPoints());
		femesh->GetPoints()->Modified();
	...
	recalc->SetBoundingBox(ugrid);
	recalc->SetRecalculateBBInteriorVertices(1);
	recalc->SetInput(femesh);
	...
	if (!callback->GetState())
	{
		femesh->GetPoints()->DeepCopy(recalc->GetOutput()->GetPoints());
		femesh->GetPoints()->Modified();
		femesh->BuildLinks();
		this->GetMimxMainWindow()->GetRenderWidget()->Render();
		this->GetMimxMainWindow()->GetRenderWidget()->ResetCamera();
		...
		recalc->Delete();
		this->GetMimxMainWindow()->SetStatusText("Recalculated interior nodes");
		return;
	}
	recalc->RemoveObserver(callback);
	recalc->Delete();
	...
	vtkUnstructuredGrid *mesh, const char *namePrefix, int index, const char *FoundationName)
{
  ...
  //this->Count++;
  vtkIdType idIndex = static_cast<vtkIdType>(index);
  meshActor->SetObjectName( namePrefix, idIndex);
                     ...
                     int numRecalc, bool generateSurfaceMesh, bool generateTriMesh, const char *FoundationName)
{
  ...
  if (!callback->GetState())
  {
    ...
    this->AddMeshToDisplay(mesh, meshPrefix.c_str(), this->BBCount, FoundationName);
    this->BBCount++;
    smooth->Delete();		
    recalc->Delete();	
...
#if 0
//---------------------------------------------------------------------------------  
int vtkKWMimxCreateFEMeshFromBBGroup::CreateMeshFromExtrusion( vtkUnstructuredGrid *originalMesh, 
                     const char *nodeSetName, int nodeNumber, const char *elementSetName, int elementNumber,
                     bool generateTriMesh, double extrusionLength, int numberOfDivisions)
{
  vtkGeomteryFilter meshSurface = vtkGeomteryFilter::New();
  meshSurface->SetInput( originalMesh );
  meshSurface->Update();
  
  /* Extract Cells */
	vtkCellData *celldata = this->UnstructuredGrid->GetCellData();
  vtkIntArray *datasetarray = vtkIntArray::SafeDownCast(celldata->GetArray( MESH_INTERNAL_EXTRUDE_SET ));
  vtkIdList *cellIds = vtkIdList::New();
  for (int i=0;i<datasetarray->GetNumberOfTuples();i++)
  {
    if (datasetarray->GetValue(i) > 0 ) cellIds->InsertNextId(i);
  }
  vtkExtractCells extractCellsFilter = vtkExtractCells::New();
  extractCellsFilter->SetInput( originalMesh );
  extractCellsFilter->SetCellList( cellIds );
  
  vtkGeomteryFilter selectSurface = vtkGeomteryFilter::New();
  selectSurface->SetInput( extractCellsFilter->GetOutput() );
  selectSurface->Update();
  
  vtkPolyData *extrudePolyData = vtkPolyData::New();
  vtkIdList *setCellPoints = vtkIdList::New();
  vtkIdList *meshCellPoints = vtkIdList::New();
  vtkPoints *setPoints = selectSurface->GetOutput()->GetPoints();
  for (int i=0;i<selectSurface->GetOutput()->GetNumberOfCells();i++)
  {
    selectSurface->GetCellPoints (i, setCellPoints);
    bool found = false
    for (int j=0;j<meshSurface->GetOutput()->GetNumberOfCells();j++)
    {
      meshSurface->GetOutput()->GetCellPoints (h, meshCellPoints);
      if (setCellPoints
    }
    if (found)
    {
														   ...
														   int NumberOfDiv, double ExtrusionLength, const char *FoundationName)
{
	...
	//vtkIntArray *selectedArray = vtkIntArray::SafeDownCast(ugrid->GetCellData()->GetArray(MESH_INTERNAL_EXTRUDE_SET));
 ...
 // 	callback->ErrorMessage("No region was selected for extrusion");
	...
	//for (int i=0;i<selectedArray->GetNumberOfTuples();i++)
	//{
	//  if (selectedArray->GetValue(i)) 
	//  {
	//    selectedIds->InsertNextId(i);
	//    numIds++;
	//  }
	//}
	//std::cout << "# IDs " << numIds << std::endl;
	//
	//vtkExtractCells *extractSelectedCells = vtkExtractCells::New();
	//extractSelectedCells->SetInput( ugrid );
	//extractSelectedCells->SetCellList( selectedIds );
	//extractSelectedCells->Update();
	//
	//vtkGeometryFilter *selectedCellGeometry = vtkGeometryFilter::New();
	//selectedCellGeometry->SetInput( extractSelectedCells->GetOutput() );
	//selectedCellGeometry->Update();	
	//vtkPolyData *sGeometry = selectedCellGeometry->GetOutput();
	//
	//vtkPolyDataWriter *tmpWriter = vtkPolyDataWriter::New();
	//tmpWriter->SetInput(sGeometry);
	//tmpWriter->SetFileName("select.vtk");
	//tmpWriter->Update( );
	//
	//vtkGeometryFilter *geofil = vtkGeometryFilter::New();
	//geofil->SetInput( ugrid );
	//geofil->Update();
	//vtkPolyData *fullGeometry = geofil->GetOutput();
	//
	//tmpWriter->SetInput(fullGeometry);
	//tmpWriter->SetFileName("full.vtk");
	//tmpWriter->Update( );
	//
	//vtkPolyData *extrudePolyData = vtkPolyData::New();
	//extrudePolyData->DeepCopy( sGeometry );
	//
	//vtkIdType *partialPts = new vtkIdType[4];
	//vtkIdType *fullPts = new vtkIdType[4];
	//for (int i=0;i<sGeometry->GetNumberOfCells();i++)
	//{
 // 	vtkIdType partialNpts;
 // 	sGeometry->GetCellPoints(i, partialNpts, partialPts);
 // 	double partialLoc[4][3];
 // 	for (int k=0;k<partialNpts;k++)
 // 	  sGeometry->GetPoint(partialPts[k], partialLoc[k]);
 // 	bool found = false;
 // 	  
 // 	for (int j=0;j<fullGeometry->GetNumberOfCells();j++)
 // 	{
 //   	vtkIdType fullNpts;
 //   	fullGeometry->GetCellPoints(i, fullNpts, fullPts);
 //   	double fullLoc[4][3];
 //   	for (int k=0;k<partialNpts;k++)
 //   	  fullGeometry->GetPoint(fullPts[k], fullLoc[k]);
 //   	if (partialNpts == fullNpts)
 //   	{
 //     	for (int k=0;k<partialNpts;k++)
 //     	{
 //     	  if ((partialLoc[k][0] == fullLoc[k][0]) || (partialLoc[k][1] == fullLoc[k][1]) || (partialLoc[k][2] == fullLoc[k][2]))
 //     	  {
 //     	    found = true;
 //     	    break;
 //     	  }
 ...
 // 	if (found == false)
 // 	{
 //   	extrudePolyData->DeleteCell(i);
 // 	}
	//}
	//delete partialPts;
	//delete fullPts;
	//extrudePolyData->RemoveDeletedCells();
	//extrudePolyData->Squeeze();
	...
	if (!callback->GetState())
	{
		...
		if(merge->MergeDataSets())
		{
			this->AddMeshToDisplay(merge->GetMergedDataSet(), "Mesh_Extrude-", this->ExtrudeCount, FoundationName);	
			this->ExtrudeCount++;
			/*
			vtkMimxMeshActor *meshActor = vtkMimxMeshActor::New();
			meshActor->SetFoundationName(FoundationName);
			...
			this->Count++;
			vtkMimxMeshActor::SafeDownCast(this->FEMeshList->GetItem(
				this->FEMeshList->GetNumberOfItems()-1))->SetObjectName("FEMesh_Extrude_",Count);
			//vtkMimxMeshActor::SafeDownCast(this->FEMeshList->GetItem(
			//	this->FEMeshList->GetNumberOfItems()-1))->GetDataSet()->Modified();
			//this->GetMimxMainWindow()->GetRenderWidget()->AddViewProp(
			//	this->FEMeshList->GetItem(this->FEMeshList->GetNumberOfItems()-1)->GetActor());
														   ...
														   int StartElemNum, bool generateTriMesh, const char *FoundationName)
{
  ...
  if ( generateTriMesh )
  {
	...
	this->AddMeshToDisplay(mesh, "Extract-", this->ExtractCount, FoundationName);
	...
	if(merge->MergeDataSets())
	{
		...
		this->AddMeshToDisplay(unsgrid, "Mesh_Extract-", this->ExtractCount, FoundationName);	
		this->ExtractCount++;
