...
vtkCxxRevisionMacro(vtkKWMimxViewPropertiesOptionGroup, "$Revision: 1.14.2.2 $");
...
{
	/*
	if(this->ViewElementSetMenuButton->GetWidget()->GetSelectedState())
	{
		this->ViewElementSetCallback(1);
	}
	if(this->ViewPropertyButton->GetWidget()->GetSelectedState())
	{
		this->ViewMaterialPropertyCallback(1);
	}
	if(this->ViewLegendButton->GetWidget()->GetSelectedState())
	{
...
#if 0
//----------------------------------------------------------------------------
void vtkKWMimxViewPropertiesOptionGroup::ViewMaterialPropertyCallback(int mode)
{
	if(mode)
	{
		if(this->ViewElementSetMenuButton->GetWidget()->GetSelectedState())
		{
			this->MeshActor->SetDisplayMode(vtkMimxMeshActor::DisplayElementSets);
			double *range = this->GetRange();
			this->MeshActor->GenerateElementSetMapperLookUpTable(
				this->ElementSetName, this->ArrayName, range);
			this->MeshActor->SetElementSetScalarVisibility(this->ElementSetName, 1);
		}
		else{
			this->MeshActor->SetDisplayMode(vtkMimxMeshActor::DisplayMesh);
			double *range = this->GetRange();
			this->MeshActor->GenerateMeshMapperLookUpTable(this->ArrayName, range);
			this->MeshActor->SetMeshScalarVisibility(1);
		}
	}
	else{
		if(this->ViewElementSetMenuButton->GetWidget()->GetSelectedState())
		{
			this->MeshActor->SetElementSetScalarVisibility(this->ElementSetName, 0);
		}else
		{
			this->MeshActor->SetMeshScalarVisibility(0);
		}
	}
	this->MimxMainWindow->RenderWidget->Render();
}
//-------------------------------------------------------------------------------
void vtkKWMimxViewPropertiesOptionGroup::ViewPropertyLegendCallback(int mode)
{
	this->MeshActor->SetMeshLegendVisibility(mode);
	this->MimxMainWindow->RenderWidget->Render();
}
//-------------------------------------------------------------------------------
void vtkKWMimxViewPropertiesOptionGroup::ViewElementSetCallback(int mode)
{
	if(mode)
	{
		this->MeshActor->SetDisplayMode(vtkMimxMeshActor::DisplayElementSets);
...
void vtkKWMimxViewPropertiesOptionGroup::DeselectAllButtons()
{
	...
	if(this->ViewElementSetMenuButton->GetWidget()->GetSelectedState())
	{
		this->ViewElementSetMenuButton->GetWidget()->SetSelectedState(0);
	}
	if(this->ViewPropertyButton->GetWidget()->GetSelectedState())
	{
...
void vtkKWMimxViewPropertiesOptionGroup::ColorModeCallback(int mode)
{
	...
	double *range = this->GetRange();
	this->MeshActor->SetColorRangeType(RangeType, this->ArrayName, this->ElementSetName, range, false);
	this->MimxMainWindow->RenderWidget->Render();
}
//-------------------------------------------------------------------------------
	 ...
	 if(this->ColorMenuButton)
	 {
		 switch ( colorType )
		 {
		   case vtkMimxMeshActor::RedToBlue:
		     ...
		     //std::cout << "Red to Blue" << std::endl;
		     break;
		   case vtkMimxMeshActor::BlueToRed: 
		     this->ColorMenuButton->GetWidget()->SetValue("Blue to Red"); 
		     //std::cout << "Blue to Red" << std::endl;
		     break;
		 }
	 }
