...
vtkCxxRevisionMacro(vtkKWMimxCreateBBFromBoundsGroup, "$Revision: 1.39.4.1 $");
//----------------------------------------------------------------------------
...
int vtkKWMimxCreateBBFromBoundsGroup::CreateBBFromBoundsApplyCallback()
{
  ...
  vtkPolyData *polydata = vtkMimxSurfacePolyDataActor::SafeDownCast(
    ...
    {
      this->Count++;
      AddBuildingBlockToDisplay(bbox->GetOutput(), "BBb-", foundationname, this->Count);
    ...
    if(this->CreateBuildingBlockFromPickWidget)
    {
  	  if(this->CreateBuildingBlockFromPickWidget->GetEnabled())
  	  {
		  ...
		  this->MaualBBCreationCount++;
  		  AddBuildingBlockToDisplay(ugrid, "BBm-", foundationname, this->MaualBBCreationCount);
	...
	vtkUnstructuredGrid *ugrid, const char *namePrefix, const char *foundationName, vtkIdType count)
{
  ...
  std::string tmpStr = namePrefix; 
  actor->SetObjectName(tmpStr.c_str(),count);
