...
vtkCxxRevisionMacro(vtkMimxMeshActor, "$Revision: 1.48.2.2 $");
...
//	return static_cast<bool> ( this->ClipPlaneGeometryFilter->GetExtractInside( ) ); 
	return this->InvertCuttingPlane;
}
...
void vtkMimxMeshActor::AddElementSetListItem( std::string setName )
{
  ...
  //vtkIntArray *scalararray = vtkIntArray::New();
  //scalararray->SetName("mimxCellScalars");
  //int numCells = elementSetProperty->ShrinkFilter->GetOutput()->GetNumberOfCells();
  //scalararray->SetNumberOfValues(numCells);
  //int i;
  //for (i=0; i<numCells; i++)
  //{
	 // scalararray->SetValue(i,i);
  //}
  //elementSetProperty->ShrinkFilter->GetOutput()->GetCellData()->AddArray(scalararray);
  //elementSetProperty->ShrinkFilter->GetOutput()->GetCellData()->SetActiveAttribute(
	 // "mimxCellScalars", vtkDataSetAttributes::SCALARS);
  //scalararray->Delete();
  //vtkUnstructuredGridWriter *writer = vtkUnstructuredGridWriter::New();
  //writer->SetFileName("Extract.vtk");
  //writer->SetInput(elementSetProperty->ShrinkFilter->GetOutput());
  //writer->Update();
  //writer->Delete();
  //
  ...
  //elementSetProperty->lutFilter = vtkLookupTable::New();

  //elementSetProperty->lutFilter->SetNumberOfColors(100);
  //elementSetProperty->lutFilter->Build();
  //double red = 0.0;
  //double green = 0.0;
  //double blue = 1.0;
  //for (i=0; i<50; i++)
  //{
	 ...
	 // elementSetProperty->lutFilter->SetTableValue(i, red*factor,green*factor,blue*factor,1.0);
	 // green = green + 0.02;
	 // blue = blue - 0.02;
  //}
  //for (i=50; i<100; i++)
...
void vtkMimxMeshActor::SetElementSetScalarName(std::string setName, std::string scalarName)
{
	...
	{
		currentSet = *it;
    //std::cout << "Set Name " << setName << " Current Name " << currentSet->name << std::endl;
		if (setName == currentSet->name)
		{
			double *range = ComputeElementSetScalarRange(setName.c_str(), scalarName.c_str());
			//std::cout << "Set scalar name " << scalarName << " for " << setName << std::endl;
  	  if ( range ) this->GenerateElementSetMapperLookUpTable(setName.c_str(), scalarName.c_str(), range);
	
  	  // currentSet->ShrinkFilter->GetOutput()->GetCellData()->SetActiveAttribute(
  		//  scalarName.c_str(),vtkDataSetAttributes::SCALARS);
  		//currentSet->ShrinkFilter->Update();
  		currentSet->SurfaceActor->Modified();
      currentSet->LegendActor->SetTitle( scalarName.c_str() );
		  break;
		}
	...
	/*
	if ( it != this->ElementSetDisplayList.end() )
	{
	  std::cout << "Set scalar name " << scalarName << " for " << setName << std::endl;
	  currentSet->ShrinkFilter->Update();
	  currentSet->ShrinkFilter->GetOutput()->GetCellData()->SetActiveAttribute(
...
void vtkMimxMeshActor::SetElementSetScalarVisibility(std::string setName, bool visibility)
{
  ...
  for ( it=this->ElementSetDisplayList.begin() ; it != this->ElementSetDisplayList.end(); it++ )
  {
    ...
    if (setName == currentSet->name)
    {
      //std::cout << "Scalar visibility " << setName << " " << visibility << std::endl;
      currentSet->SurfaceMapper->SetScalarVisibility(static_cast<int>(visibility));
      currentSet->SurfaceActor->Modified();
      break;
    }
	...
	const char *ElementSetname, const char *ArrayName, double *range)
{
	...
	//writer->SetFileName("Extract1.vtk");
	//writer->SetInput(ugrid);
	//writer->Write();
	//writer->Delete();
	this->lutFilter->SetTableRange(range[0], range[1]);
	...
	//this->SetDisplayMode(vtkMimxMeshActor::DisplayElementSets);
	//writer = vtkUnstructuredGridWriter::New();
	//writer->SetFileName("Extract2.vtk");
	//writer->SetInput(ugrid);
	//writer->Write();
	//writer->Delete();
	//writer = vtkUnstructuredGridWriter::New();
	//writer->SetFileName("Extract3.vtk");
	//writer->SetInput(ugrid);
	//writer->Write();
	//writer->Delete();
}
...
{
	//if(this->ColorRangeType == RangeType)	return;
	/*else	*/
	this->ColorRangeType = RangeType;
	if ((this->DisplayMode == vtkMimxMeshActor::DisplayMesh) || scaleEntireMesh)
	{
